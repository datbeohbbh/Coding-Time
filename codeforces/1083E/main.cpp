...
namespace task{
    ...
    public:
        ...
        llong get(const int &x){
            ...
            for(int mid = (l + r) >> 1;r - l > 1;mid = (l + r) >> 1){
            ...
            return max(hull[l].evaluate(x),hull[r].evaluate(x));
        }
    };
    ...
    void solve(){
        ...
        for(int i = 0;i < n;++i){
            f[i] = max(0LL,1LL * rect[i].first.first * 1LL * rect[i].first.second - rect[i].second + cht -> get(rect[i].first.second));
